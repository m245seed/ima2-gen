...
import { OPENAI_IMAGE_MODEL_OPTIONS, UNSUPPORTED_IMAGE_MODELS } from "../lib/imageModels";
...
export function ImageModelSelect({ variant }: ImageModelSelectProps) {
  ...
  const modelOptions = OPENAI_IMAGE_MODEL_OPTIONS;
  const current = modelOptions.find((option) => option.value === imageModel) ?? modelOptions[0];
  ...
  if (variant === "sidebar") {
    const triggerModel = current.shortLabel;
    return (
      <div ref={rootRef} className="image-model-select image-model-select--sidebar">
        <button
          ...
          aria-haspopup="menu"
          aria-expanded={open}
          aria-label={t("sidebar.quickSettingsAria", { model: triggerModel, effort: currentReasoning.shortLabel })}
            ...
            <details className="image-model-select__section image-model-select__collapsible">
              ...
              <div role="group" aria-label={t("sidebar.reasoningLabel")}>
                {REASONING_EFFORT_OPTIONS.map((option, index) => (
                  <button
                    key={option.value}
                    ref={(node) => { menuItemRefs.current[modelOptions.length + index] = node; }}
                    type="button"
