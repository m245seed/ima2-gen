...
test("classic, multimode, and node stream clients use capacity retry submitter", () => {
  ...
  for (const endpoint of ["/api/generate", "/api/generate/multimode"]) {
    assert.match(generation, new RegExp(`url: "${endpoint.replaceAll("/", "\\/")}"`));
  }
  assert.match(nodeApi, /url: "\/api\/node\/generate"/);

  assert.equal((generation.match(/submitAsyncJobWithCapacityRetry/g) ?? []).length, 3);
  assert.equal((nodeApi.match(/submitAsyncJobWithCapacityRetry/g) ?? []).length, 2);
  assert.equal((generation.match(/submitController\.abort\(\)/g) ?? []).length, 6);
